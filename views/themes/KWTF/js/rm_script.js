jQuery(document).ready(function(){
		...
		jQuery('.section_effect .rm_options:not(.opened)').css({ display: 'none'});
		...
		jQuery('.section_effect .rm_title h3').click(function(){
			...
			jQuery(this).parent().next('.rm_options').slideToggle('slow', function(){
                optionsTab.redraw();
            });
		});
		
		jQuery('input[type="checkbox"].on_off').iphoneStyle({ checkedLabel: 'Вкл', uncheckedLabel: 'Выкл' });
    });                   	
