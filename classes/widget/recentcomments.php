 ...
 * Date : 22.09.12
 * Time : 19:31
 * File : recentComments.php
 * @name recentComments
 * @packages Wordpress/ThemeFramework/recentComments
 * @subpackage Widget
 * @category Widget
 ...
 */
class Widget_RecentComments  extends WPWidget
{
  ...
  function __construct()
  {
    $widget_ops = array(
      ...
      'description' => __( 'The most recent comments.', self::$meta[WordpressTheme::OPT_WIDGET_TDOMAIN] )
    );

    $control_ops = array( 'id_base' => 'recent-comments' );

    $this->defaults = array(
      'title' => __( 'Recent Comments', self::$meta[WordpressTheme::OPT_WIDGET_TDOMAIN] ),
      'number' => 5,
      'show_avatar' => 'yes',
      'show_author' => 'yes',
      'show_content' => 'yes',
    ...
    parent::__construct( 'recent-comments', $widget_ops, $control_ops );
  }
  ...
  <p><label for="<?php echo $this->get_field_id('show_avatar'); ?>"><?php _e('Show avatar:'); ?></label>
    <select id="<?php echo $this->get_field_id('show_avatar'); ?>" name="<?php echo $this->get_field_name('show_avatar'); ?>">
      <option value="yes"<?php selected( $instance['show_avatar'], 'yes' ) ?>><?php _e( 'Yes', $this->_meta[WordpressTheme::OPT_WIDGET_TDOMAIN] ) ?></option>
      <option value="no"<?php selected( $instance['show_avatar'], 'no' ) ?>><?php _e( 'No', $this->_meta[WordpressTheme::OPT_WIDGET_TDOMAIN] ) ?></option>
    </select>
  </p>
  ...
  <p><label for="<?php echo $this->get_field_id('show_author'); ?>"><?php _e('Show author:'); ?></label>
    <select id="<?php echo $this->get_field_id('show_author'); ?>" name="<?php echo $this->get_field_name('show_author'); ?>">
      <option value="yes"<?php selected( $instance['show_author'], 'yes' ) ?>><?php _e( 'Yes', $this->_meta[WordpressTheme::OPT_WIDGET_TDOMAIN] ) ?></option>
      <option value="no"<?php selected( $instance['show_author'], 'no' ) ?>><?php _e( 'No', $this->_meta[WordpressTheme::OPT_WIDGET_TDOMAIN] ) ?></option>
    </select>
  </p>
  ...
  <p><label for="<?php echo $this->get_field_id('show_content'); ?>"><?php _e('Show comment text:'); ?></label>
    <select id="<?php echo $this->get_field_id('show_content'); ?>" name="<?php echo $this->get_field_name('show_content'); ?>">
      <option value="yes"<?php selected( $instance['show_content'], 'yes' ) ?>><?php _e( 'Yes', $this->_meta[WordpressTheme::OPT_WIDGET_TDOMAIN] ) ?></option>
      <option value="no"<?php selected( $instance['show_content'], 'no' ) ?>><?php _e( 'No', $this->_meta[WordpressTheme::OPT_WIDGET_TDOMAIN] ) ?></option>
    </select>
  </p>
  ...
  function widget( $args, $instance )
  {
    ...
    foreach( $comments as $comment ) :
      ...
      <div class="the-post group">
        ...
        <?php if ( $content == 'yes' ) : ?>
        <p class="comment">
          <?php $this->_plugin->excerpt_text( strip_tags( $comment->comment_content ), $excerpt_length, '...' ) ?>
